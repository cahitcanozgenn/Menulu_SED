...
#include <conio.h>
#include <windows.h>
//Cahit Can OZGEN | Software Developer
...
void PBaslik()
    { clrscr();
      color(2);
      gotoxy( 1, 1); printf("_(mkl)_");
      gotoxy(11, 1); printf("Menulu SED ile Dosyalama");
      gotoxy(44, 1); printf("Vers: 3.00");
    }
void Pmenu ()
    { PBaslik();
    color(3);
      gotoxy(11, 5); printf("---_MENU_---");
      gotoxy(11, 6); printf("1. D.K. Yaz");
      gotoxy(11, 7); printf("2. D.K. AraBulGoster");
      gotoxy(11, 8); printf("3. D.K. Sil");
      gotoxy(11, 9); printf("4. D.K. Duzelt");
      gotoxy(11,10); printf("5. D.K. Listele");
...
void BilGos () 
    { gotoxy(22, 5); printf("%d" , ODBlg.nmr);
      gotoxy(22, 6); printf("%s" , ODBlg.adi);
      gotoxy(22, 7); printf("%s" , ODBlg.syd);
      gotoxy(22, 8); printf("%s" , ODBlg.tlf);
      gotoxy(22, 9); printf("%d" , ODBlg.yas);
    }
void BilGir ()
    { BilEkr();
    color(1);
...
Miste:
	...
	if (sec==0) goto Son;
	if (sec>5) { gotoxy(11,20); color(2); printf("Hatali Secim Yapildi, Tekrar Giriniz."); goto Miste; }
	if (sec==1) goto DkYaz;
	if (sec==2) goto DkOku;
	if (sec==3) goto DkSil;
	if (sec==4) goto DkDuz;
...
DkYaz:
	...
	BilGos(); 
	ODAcY(); ODyaz(); fclose(OgrDos);
	gotoxy(11,21); color(1); printf("Kayit YAZILDI.");
	gotoxy(11,22); printf("MenuÅicin ENTER Tuslayiniz: "); getch();
DKYcik:
	goto Menu;
DkOku:
	PBaslik();
...
DkSil:
	...
	remove("ogrX.dat");  rename("gecX.dat","ogrX.dat");
	if (bul==0) { gotoxy(11,20); printf("BULUNAMADI..."); }
	gotoxy(11,22); printf("Menu icin ENTER Tuslayiniz:"); getch();
DKScik:
	goto Menu;
DkDuz:
	...
	fclose(OgrDos);	 fclose(GecDos);
	remove("ogrX.dat");  rename("gecX.dat","ogr.dat");
	if (bul==0) { gotoxy(11,20); printf("BULUNAMADI..."); }
	gotoxy(11,22); printf("Menu icin ENTER Tuslayiniz:"); getch();
DKDcik:
	goto Menu;
